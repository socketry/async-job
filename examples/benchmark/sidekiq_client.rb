...
def benchmark_sidekiq(job_count = 10_000)
	puts "Benchmarking sidekiq with #{job_count} jobs..."
	
	start_time = Process.clock_gettime(Process::CLOCK_MONOTONIC)
	
	job_count.times do |index|
		SidekiqBenchmarkJob.perform_later(index)
	end
	
	end_time = Process.clock_gettime(Process::CLOCK_MONOTONIC)
	
	puts "Sidekiq results:"
